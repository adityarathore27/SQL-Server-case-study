...
delete  dim_manufacturer
truncate table dim_manufacturer
...
(85086, 'US', 'Arizona', 'Anthem'),
(85117, 'US', 'Arizona', 'Apache Jct'), 
(94005, 'US', 'California', 'Brisbane'), 
(92703, 'US', 'California', 'Bristol'), 
(21163, 'US', 'Maryland', 'Woodstock'),
(21648, 'US', 'Maryland', 'Woolford'), 
(122002, 'India', 'Haryana', 'Gurgaon'),
(530068, 'India', 'Karnataka', 'Bangalore'), 
(110004, 'India', 'Delhi', 'Delhi'),
(400006, 'India', 'Maharashtra', 'Mumbai')
...
--creating table fact_transaction
create table fact_transactions
(

ID_model  int   references dim_model(ID_model),
...
--solution 1: 
              select distinct dl.IDlocation from dim_location as dl join fact_transactions as ft
 ...
 --solution 2 :
                ...
                select distinct dl.state from dim_model as dm join fact_transactions as ft
				                on dm.ID_model=ft.ID_model
								join dim_location as dl
								on dl.IDlocation=ft.IDlocation
								join dim_manufacturer as dmf
								on dmf.IDmanufacturer=dm.IDmanufacturer
								where dl.state='US' and dmf.manufacturer_name='samsung'

				--both are correct
				select distinct dl.state from dim_location as dl join fact_transactions as ft
				                       on dl.IDlocation=ft.IDlocation
...
--solution 3 
          ...
          select count(*) as [no of transaction] from
		  (select distinct ft.ID_model,dl.zipcode,dl.state
		          from dim_location as dl join fact_transactions as ft
				  on dl.IDlocation=ft.IDlocation
				  ) as TT
...
--solution 4
					 ...
					 select * from dim_customer
					  ...
					  select row_number() over(order by unit_price asc) , ID_model,model_name,unit_price from dim_model
					 --solution 
                     select model_name,unit_price,ID_model from dim_model
					 where unit_price=(
 ...
 --solution 5)    
                ...
                select top (5) avg(unit_price) from dim_model
				group by model_name

				select top (5) avg(unit_price) as [avg price],model_name,quantity from dim_model as dm join fact_transactions as ft
				on dm.ID_model=ft.ID_model
 ...
 --solution 6
		...
		select dc.customer_name,avg(ft.totalprice) as [avg_price] from dim_customer as dc join fact_transactions as ft
		on dc.IDcustomer=ft.ID_model
 ...
 --solution: 7
                  ...
                  select top(5) row_number() over(partition by year order by quantity asc) as [quantity]
				  from fact_transactions 
				  where year(fact_transactions) in ('2008','2009','2010')
